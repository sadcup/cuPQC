...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    std::cout << "================================================================\n";
    std::cout << "ML-KEM (Module-Lattice Key Encapsulation Mechanism) Example\n";
    std::cout << "================================================================\n\n";
    
    std::cout << "This example demonstrates ML-KEM (formerly CRYSTALS-Kyber), a\n";
    std::cout << "post-quantum key encapsulation mechanism standardized in NIST\n";
    std::cout << "FIPS 203. ML-KEM provides secure key exchange that is resistant\n";
    std::cout << "to attacks from both classical and quantum computers. This example\n";
    std::cout << "demonstrates batch processing of key generation, encapsulation, and\n";
    std::cout << "decapsulation operations using GPU acceleration.\n\n";
    
    std::cout << "Configuration: ML-KEM-512, batch size: 10 key pairs\n\n";
    ...
    std::cout << "========================================\n";
    std::cout << "Generating " << batch << " key pairs...\n";
    /*
    ...
    std::cout << "========================================\n";
    std::cout << "Encapsulating shared secrets for " << batch << " public keys...\n";
    /*
    ...
    std::vector<uint8_t> sharedsecrets_encap(MLKEM512Encaps::shared_secret_size * batch);
    ml_kem_encaps(ciphertexts, sharedsecrets_encap, public_keys, batch);
    std::cout << "Encapsulation completed.\n\n";

    std::cout << "========================================\n";
    std::cout << "Decapsulation\n";
    std::cout << "========================================\n";
    std::cout << "Decapsulating " << batch << " ciphertexts...\n";
    /*
     * Decapsulate ciphertext and store into shared secret!
     */
    std::vector<uint8_t> sharedsecrets_decap(MLKEM512Encaps::shared_secret_size * batch);
    ml_kem_decaps(sharedsecrets_decap, ciphertexts, secret_keys, batch);
    std::cout << "Decapsulation completed.\n\n";
    
    std::cout << "========================================\n";
    std::cout << "Verification\n";
    std::cout << "========================================\n";
    // Verify that encapsulated and decapsulated shared secrets match
    // In ML-KEM: Encapsulation generates K and ct, Decapsulation recovers K' from ct
    // For correct operation, K should equal K' when using matching key pairs
    ...
    for (size_t i = 0; i < batch; i++) {
        ...
        for (size_t j = 0; j < MLKEM512Encaps::shared_secret_size; j++) {
            if (sharedsecrets_encap[i * MLKEM512Encaps::shared_secret_size + j] != 
                sharedsecrets_decap[i * MLKEM512Encaps::shared_secret_size + j]) {
                match = false;
                first_diff = j;
                break;
            }
        ...
        if (!match) {
            std::cout << "Key pair " << i << " - MISMATCH at byte " << first_diff << ":\n";
            std::cout << "  Encapsulated shared secret (first 16 bytes): ";
            size_t bytes_to_show = (16 < MLKEM512Encaps::shared_secret_size) ? 16 : MLKEM512Encaps::shared_secret_size;
            for (size_t j = 0; j < bytes_to_show; j++) {
                std::cout << std::setw(2) << static_cast<unsigned>(sharedsecrets_encap[i * MLKEM512Encaps::shared_secret_size + j]) << " ";
            }
            std::cout << "\n  Decapsulated shared secret (first 16 bytes): ";
            for (size_t j = 0; j < bytes_to_show; j++) {
                std::cout << std::setw(2) << static_cast<unsigned>(sharedsecrets_decap[i * MLKEM512Encaps::shared_secret_size + j]) << " ";
            }
            std::cout << "\n";
        }
    ...
    std::cout << "\nShared secret verification: " << match_count << " / " << batch << " matched\n";
    if (match_count == batch) {
        std::cout << "✓ All shared secrets match - encapsulation/decapsulation working correctly!\n";
    } else {
        std::cout << "✗ Warning: " << (batch - match_count) << " shared secrets do not match\n";
    }
    std::cout << "\n";
    
    std::cout << "ML-KEM operations completed successfully.\n";
}
