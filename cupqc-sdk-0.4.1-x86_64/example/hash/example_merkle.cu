...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    single_verify_kernel<MERKLE, HASH, uint32_t><<<1, 1>>>(first_proof, leaf_to_prove, 0, d_root, d_verified);
    ...
    verify_proof<MERKLE, HASH, uint32_t>(d_leaves_to_prove, d_indices_to_prove, 256, proofs, d_root, d_verified);
    for (size_t i = 0; i < 256; i++) {
